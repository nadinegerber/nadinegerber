
# Hi! 👋 🇿🇦

### 👩‍💻 Welcome!


Based in Cape Town and focused on becoming an **AI Developer**  specializing in the building of autonomous agentic systems, custom automation tools, and localized data pipelines.....to turn data into kick-ass business intelligence. 

As a **Creative Problem Solver** I approach development with a **code-first, lightweight philosophy**— building modular, efficient systems from scratch using Python and terminal-driven environments rather than relying on heavy graphical user interfaces (GUIs).


Already I am loving **conceptualizing** and **building** AI tools that can solve real-world problems....by focusing on bridging the gap between local logic and cloud-based AI solutions. 


I believe that the mark of a great Product Designer— is prioritizing "Usability" over "Aesthetics" because a design which causes users to hunt for features or functionality to proceed, will cause frustration and vacation of the application. Consistency and ease of use always win.

And so too the mark of a great developer is: asking "Why?" before asking "How?" The answer depends entirely on the Business Goal. One could build a tool to serve two completely different purposes, or even a mix of both.

At the end of the day...the goal is not to be a consumer of technology, but an architect of it.

<img width="640" height="640" alt="nadine" src="https://github.com/user-attachments/assets/77a873b6-20bf-4458-9fe1-6efef7dc5169" />

---

### 🚀 Current Fun Projects

<br>

#### 👁️ Blink & Move - An 20-20-20 Interval Timer (Ergonomic Focus Engine)
A local desktop automation and productivity utility designed to combat digital eye strain and prolonged sedentary behavior through an interactive, language-learning loop. Features a thread-safe, native graphical interface with zero standby overhead, robust local data-persistence scheduling, and synchronized audio playback.
* Built with: Python (Tkinter & Multi-threading), macOS Speech Synthesis (Thomas Engine), Local file storage.

<br>

#### 🎮 The Dopamine Dealer (A Premium Skeuomorphic Productivity Engine)
A local, gamified micro-server that turns real-world task execution into an interactive RPG experience. Features an isolated backend API framework with dynamic client-side DOM updates and localized environment security variable controls.
A self-contained, gamified micro-server architecture that transforms physical task execution into a high-fidelity, interactive console experience. Features a localized backend routing pipeline, automated state management triggers, and a custom-tuned skeuomorphic interface utilizing stacked emissive LED light modeling for real-time visual feedback.

*   **Engine Core:** Python (Base HTTP Server Framework)
*   **Data Integrity:** Localized JSON Database Storage Architecture
*   **Interface Layer:** Modular HTML5 / Vanilla JavaScript (Async DOM Manipulation)
*   **Aesthetic Frame:** Custom CSS3 Skeuomorphic Matrix (Tactile Key Modeling & Stacked Glow Mechanics)
  
<br>

#### 🇫🇷 French Tutor App (An Intelligent Language Companion)
A database-backed, agentic full-stack web application that combines local text processing with real-time AI morphological analysis. Features a skeuomorphic interface, automated PostgreSQL edge caching, and a low-latency pipeline that interfaces directly with Groq cloud infrastructure.
* *Built with:* Python (Base HTTP server), PostgreSQL, Llama 3.1 Inference Engine, Frontend HTML5/Skeuomorphic CSS3.

<br>

<img width="1772" height="1772" alt="nadine" src="https://github.com/user-attachments/assets/f6fc91b9-7ad5-4479-906f-69658ed40b5e" />

---

### 🛠 Tech Stack & Toolkit - Currently (yes I am starting way at the bottom) 

* **Languages:** Python, HTML5, CSS3, JavaScript (Foundational)
* **AI:** Google Gemini API, Llama 3.1 Inference Engine
* **Development Environment:** Terminal, Sublime Text, Git/GitHub
* **Operating Systems:** macOS (Legacy & Modern environment optimization)
* **Workflow:** Terminal, Git/GitHub, Sublime Text
* **Frontend:** Streamlit, HTML, CSS 
<!--* **AI Frameworks:** LangChain, CrewAI-->

---

### 🇿🇦 About me
* 📍 Based in: **Cape Town, South Africa**
* Currently exploring agent architecture. Expanding knowledge and technical capabilities in algorithmic systems. Studying French.
* 🚲 When I'm not coding: You'll find me out on my road bike doing long solo rides! I managed to do a solo 270km ride in December 2025. Aiming for a solo 350km in December 2026 (God willing).

<img width="216" height="444" alt="Nadine-cycling" src="https://github.com/user-attachments/assets/98d60282-4bab-4e9d-b73d-d5d6abda3a6f" />

<img width="216" height="444" alt="Nadine-cycling-b" src="https://github.com/user-attachments/assets/b8f5cd45-e135-4a9d-9510-f3a5fce95d14" />

  ---
> *"The goal is not to be a consumer of technology, but an architect of it."*










<!--
### 🎯 Focus & Professional Objectives

I am actively focused on mastering the engineering principles behind autonomous AI agents, tool-calling loops, and custom logic systems. My objective is to deliver high-performing, secure, and independent AI solutions for automation, data analysis, and advanced workflow orchestration.

---
-->



<!--
**nadinegerber/nadinegerber** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
